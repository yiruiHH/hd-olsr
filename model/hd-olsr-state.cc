...
/********** Highly Dynamic Set Manipulation **********/

HdTuple*
OlsrState::FindHdTuple (Ipv4Address const &mainAddr)
{
  for (HdSet::iterator it = m_hdSet.begin ();
       it != m_hdSet.end (); it++)
    {
      if (it->neighborMainAddr == mainAddr)
        {
...
void
OlsrState::EraseHdTuple (const HdTuple &tuple)
{
  for (HdSet::iterator it = m_hdSet.begin ();
       it != m_hdSet.end (); it++)
    {
...
void
OlsrState::EraseHdTuple (const Ipv4Address &mainAddr)
{
  for (HdSet::iterator it = m_hdSet.begin ();
       it != m_hdSet.end (); )
    {
...
void
OlsrState::InsertHdTuple (HdTuple const &tuple)
{
  HdTuple* t = this->FindHdTuple(tuple.neighborMainAddr);

  if(t!=NULL)
  {
    this->EraseHdTuple(tuple);
  }
  
  m_hdSet.push_back (tuple);
  
}

std::string
OlsrState::PrintHdSet() const
{
  std::ostringstream os;
  os << "[\n";
  for(HdSet::const_iterator iter = m_hdSet.begin();
      iter != m_hdSet.end(); iter++)
    {
      HdSet::const_iterator next = iter;
      next++;
      os << *iter;
      if (next != m_hdSet.end())
      {
