...
#define HD_MESSAGE_SIZE 48

namespace ns3 {

NS_LOG_COMPONENT_DEFINE ("HDOlsrHeader");
...
{
  static TypeId tid = TypeId ("ns3::hdolsr::PacketHeader")
    .SetParent<Header> ()
    .SetGroupName ("HDOlsr")
...
{
  static TypeId tid = TypeId ("ns3::hdolsr::MessageHeader")
    .SetParent<Header> ()
    .SetGroupName ("HDOlsr")
...
MessageHeader::GetSerializedSize (void) const
{
  ...
  switch (m_messageType)
    ...
    case HNA_MESSAGE:
      ...
      break;
    case HD_MESSAGE:
      size += m_message.hd.GetSerializedSize ();
      break;
    default:
      NS_ASSERT (false);
    }
...
MessageHeader::Deserialize (Buffer::Iterator start)
{
  ...
  NS_ASSERT (m_messageType >= HELLO_MESSAGE && m_messageType <= HD_MESSAGE);
  m_vTime  = i.ReadU8 ();
  ...
  switch (m_messageType)
    ...
    case HNA_MESSAGE:
      ...
      break;
    case HD_MESSAGE:
      size += m_message.hd.Deserialize (i, m_messageSize - OLSR_MSG_HEADER_SIZE);
      break;
    default:
      NS_ASSERT (false);
    }
...
uint32_t
MessageHeader::HD::GetSerializedSize(void) const
{
  return HD_MESSAGE_SIZE;
}

void
MessageHeader::HD::Print (std::ostream &os) const
{
  os << "\nHD_MESSAGE\n" 
  << "Location: " << m_location
...
void
MessageHeader::HD::Serialize (Buffer::Iterator start) const
{
  Buffer::Iterator i = start;

  i.Write((uint8_t*)&m_location,24);
  i.Write((uint8_t*)&velocity,24);

  NS_LOG_DEBUG("Serialize:location:" << m_location <<"velocity: " << velocity);

}

uint32_t
MessageHeader::HD::Deserialize(Buffer::Iterator start, uint32_t messageSize)
{
  Buffer::Iterator i = start;

  NS_ASSERT(messageSize==HD_MESSAGE_SIZE);

  i.Read((uint8_t*)&(this->m_location),24);
  i.Read((uint8_t*)&(this->velocity),24);

  NS_LOG_DEBUG("Deserialize:location:" << m_location <<"velocity: " << velocity);

  return messageSize;
}

}
}  // namespace hdolsr, ns3
