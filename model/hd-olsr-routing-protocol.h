...
#ifndef HD_OLSR_AGENT_IMPL_H
#define HD_OLSR_AGENT_IMPL_H

#include "hd-olsr-header.h"
#include "ns3/test.h"
#include "hd-olsr-state.h"
#include "hd-olsr-repositories.h"
...
public:
   ...
   */
  void Dump (void);
  void PrintMprSet (Ptr<OutputStreamWrapper> stream, Time::Unit unit = Time::S) const;
  int GetMprNums();
  /**
...
private:
  ...
  Time m_hdInterval;      //!< HD messages' emission interval.
  ...
  //virtual void PrintMprSet (Ptr<OutputStreamWrapper> stream, Time::Unit unit = Time::S) const;
  ...
  Timer m_hdTimer; //!< Timer for the HNA message.
  /**
   * \brief Sends an HNA message (if the node has associated hosts/networks) and reschedules the HNA timer.
   */
  void HdTimerExpire ();
  ...
  hdolsr::MessageList m_queuedMessages;
   ...
   */
  void ForwardDefault (hdolsr::MessageHeader olsrMessage,
   ...
   */
  void QueueMessage (const hdolsr::MessageHeader &message, Time delay);
  ...
  /**
   * \brief Creates a new %OLSR HD message which is buffered for being sent later on.
   */
  void SendHd ();
   ...
   */
  void ProcessHello (const hdolsr::MessageHeader &msg,
   ...
   *
   */
  void ProcessTc (const hdolsr::MessageHeader &msg,
                  const Ipv4Address &senderIface);
   ...
   * \param senderIface the address of the interface where the message was sent from.
   */
  void ProcessMid (const hdolsr::MessageHeader &msg,
                   const Ipv4Address &senderIface);
   ...
   *
   */
  void ProcessHna (const hdolsr::MessageHeader &msg,
                   const Ipv4Address &senderIface);
  
  /**
   *
   * \brief Processes a HD message.
   *
   * The Highly Dynamic Association Set is updated (if needed) with the information
   * of the received HD message.
   *
   * \param msg the %OLSR message which contains the HD message.
   * \param senderIface the address of the interface where the message was sent from.
   *
   */
  void ProcessHd (const hdolsr::MessageHeader &msg,
                   const Ipv4Address &senderIface);
   ...
   */
  void LinkSensing (const hdolsr::MessageHeader &msg,
   ...
   * \param hello The received HELLO sub-message.
   */
  void PopulateNeighborSet (const hdolsr::MessageHeader &msg,
                            const hdolsr::MessageHeader::Hello &hello);
   ...
   * \param hello The received HELLO sub-message.
   */
  void PopulateTwoHopNeighborSet (const hdolsr::MessageHeader &msg,
                                  const hdolsr::MessageHeader::Hello &hello);
   ...
   * \param hello The received HELLO sub-message.
   */
  void PopulateMprSelectorSet (const hdolsr::MessageHeader &msg,
                               const hdolsr::MessageHeader::Hello &hello);

  /**
   * \brief Updates the Neighbor Set according to the information contained in
   * a new received HD message.
   * \param msg The received message.
   * \param hello The received HD sub-message.
   */
  void PopulateNeighborWillingness (const hdolsr::MessageHeader &msg,
