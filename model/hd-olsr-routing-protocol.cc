...
#include "hd-olsr-routing-protocol.h"
...
/// HD holding time.
#define OLSR_HD_HOLD_TIME      Time (3 * m_hdInterval)
...
#define MIN_SPEED_DIFF          0

#define MAX_SPEED_DIFF          (2 * m_max_speed)

#define MIN_NODE_SPEED          m_min_speed

#define MAX_NODE_SPEED          m_max_speed

#define WILL_TRANS_FACTOR       (MAX_SPEED_DIFF/6)

#define MAX_RANGE               m_max_range

#define MAX_T_LD                MAX_RANGE / MAX_NODE_SPEED

namespace ns3 {

NS_LOG_COMPONENT_DEFINE ("HDOlsrRoutingProtocol");
...
{
  static TypeId tid = TypeId ("ns3::hdolsr::RoutingProtocol")
    ...
    .AddAttribute ("HnaInterval", "HNA messages emission interval.  Normally it is equal to TcInterval.",
                   ...
                   MakeTimeChecker ())
    .AddAttribute ("HdInterval", "HD messages emission interval.",
                   TimeValue (Seconds (1)),
                   MakeTimeAccessor (&RoutingProtocol::m_hdInterval),
                   MakeTimeChecker ())
    ...
    .AddAttribute ("MinSpeed", "Min node speed",
                    DoubleValue (20),
                    MakeDoubleAccessor (&RoutingProtocol::m_min_speed),
                    MakeDoubleChecker<double> ())
    .AddAttribute ("MaxSpeed", "Max node speed",
                    DoubleValue (30),
                    MakeDoubleAccessor (&RoutingProtocol::m_max_speed),
                    MakeDoubleChecker<double> ())
    .AddAttribute ("MaxRange", "Max node range",
                    DoubleValue (300),
                    MakeDoubleAccessor (&RoutingProtocol::m_max_range),
                    MakeDoubleChecker<double> ())
    .AddTraceSource ("Rx", "Receive OLSR packet.",
                     MakeTraceSourceAccessor (&RoutingProtocol::m_rxPacketTrace),
                     "ns3::hdolsr::RoutingProtocol::PacketTxRxTracedCallback")
    ...
    .AddTraceSource ("RoutingTableChanged", "The OLSR routing table has changed.",
                     ...
                     "ns3::hdolsr::RoutingProtocol::TableChangeTracedCallback")
  ;
  return tid;
}
...
RoutingProtocol::RoutingProtocol ()
  ...
  m_hdTimer (Timer::CANCEL_ON_DESTROY),
...
RoutingProtocol::SetIpv4 (Ptr<Ipv4> ipv4)
{
  NS_ASSERT (ipv4 != 0);
  NS_ASSERT (m_ipv4 == 0);
  NS_LOG_DEBUG ("Created hdolsr::RoutingProtocol");
  ...
  m_hdTimer.SetFunction (&RoutingProtocol::HdTimerExpire, this);
...
void
RoutingProtocol::PrintMprSet (Ptr<OutputStreamWrapper> stream, Time::Unit unit) const
{
  std::ostream* os = stream->GetStream ();
  ...
  *os << "Node: " << m_ipv4->GetObject<Node> ()->GetId ()
      << ", Time: " << Now ().As (unit)
      << ", Local time: " << GetObject<Node> ()->GetLocalTime ().As (unit)
      << ", OLSR Mpr Set" << std::endl;
  
  for(std::set<Ipv4Address>::const_iterator iter = m_state.GetMprSet().begin(); 
      iter != m_state.GetMprSet().end(); iter++)
  {
    *os << *iter << " ";
  }
      
  *os << "mpr num = " << m_state.GetMprSet().size() << "\n";
}

int
RoutingProtocol::GetMprNums()
{
...
RoutingProtocol::RecvOlsr (Ptr<Socket> socket)
{
  ...
  hdolsr::PacketHeader olsrPacketHeader;
      ...
      if (duplicated == NULL)
        {
          switch (messageHeader.GetMessageType ())
            {
            case hdolsr::MessageHeader::HELLO_MESSAGE:
            ...
            case hdolsr::MessageHeader::TC_MESSAGE:
              NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                            << "s OLSR node " << m_mainAddress
            ...
            case hdolsr::MessageHeader::MID_MESSAGE:
              NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                            << "s OLSR node " << m_mainAddress
                            <<  " received MID message of size " << messageHeader.GetSerializedSize ());
              ProcessMid (messageHeader, senderIfaceAddr);
              break;
            case hdolsr::MessageHeader::HNA_MESSAGE:
              NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                            << "s OLSR node " << m_mainAddress
                            <<  " received HNA message of size " << messageHeader.GetSerializedSize ());
              ProcessHna (messageHeader, senderIfaceAddr);
              break;
            case hdolsr::MessageHeader::HD_MESSAGE:
              NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                            << "s OLSR node " << m_mainAddress
                            <<  " received HD message of size " << messageHeader.GetSerializedSize ());
              ProcessHd (messageHeader, senderIfaceAddr);
              break;
      ...
      if (do_forwarding)
        {
          ...
          if (messageHeader.GetMessageType () != hdolsr::MessageHeader::HELLO_MESSAGE &&
              messageHeader.GetMessageType () != hdolsr::MessageHeader::HD_MESSAGE)
            {
...
void
RoutingProtocol::ProcessHello (const hdolsr::MessageHeader &msg,
                               const Ipv4Address &receiverIface,
                               const Ipv4Address &senderIface)
{
  NS_LOG_FUNCTION (msg << receiverIface << senderIface);

  const hdolsr::MessageHeader::Hello &hello = msg.GetHello ();
...
void
RoutingProtocol::ProcessTc (const hdolsr::MessageHeader &msg,
                            const Ipv4Address &senderIface)
{
  const hdolsr::MessageHeader::Tc &tc = msg.GetTc ();
...
void
RoutingProtocol::ProcessMid (const hdolsr::MessageHeader &msg,
                             const Ipv4Address &senderIface)
{
  const hdolsr::MessageHeader::Mid &mid = msg.GetMid ();
  Time now = Simulator::Now ();
...
void
RoutingProtocol::ProcessHna (const hdolsr::MessageHeader &msg,
                             const Ipv4Address &senderIface)
{

  const hdolsr::MessageHeader::Hna &hna = msg.GetHna ();
  Time now = Simulator::Now ();
  ...
  for (std::vector<hdolsr::MessageHeader::Hna::Association>::const_iterator it = hna.associations.begin ();
...
void
RoutingProtocol::ProcessHd(const hdolsr::MessageHeader &msg,
                           const Ipv4Address &senderIface)
{
  const hdolsr::MessageHeader::HD &hd = msg.GetHd ();
  Time now = Simulator::Now();

  NS_LOG_DEBUG(now.GetSeconds() << " Node " << m_mainAddress << " process HD message from " << senderIface);
  ...
  NS_LOG_DEBUG(hdtuple);

  //NS_LOG_DEBUG(m_state.PrintHdSet());

  m_state.InsertHdTuple(hdtuple);
  PopulateNeighborWillingness(msg,hd);
}


void
RoutingProtocol::ForwardDefault (hdolsr::MessageHeader olsrMessage,
...
void
RoutingProtocol::QueueMessage (const hdolsr::MessageHeader &message, Time delay)
{
...
RoutingProtocol::SendQueuedMessages ()
{
  ...
  for (std::vector<hdolsr::MessageHeader>::const_iterator message = m_queuedMessages.begin ();
...
RoutingProtocol::SendHello ()
{
  ...
  hdolsr::MessageHeader::Hello &hello = msg.GetHello ();

  hello.SetHTime (m_helloInterval);
  hello.willingness = m_willingness;

  std::vector<hdolsr::MessageHeader::Hello::LinkMessage>
  ...
  for (LinkSet::const_iterator link_tuple = links.begin ();
       link_tuple != links.end (); link_tuple++)
    {
      ...
      hdolsr::MessageHeader::Hello::LinkMessage linkMessage;
...
RoutingProtocol::SendTc ()
{
  ...
  hdolsr::MessageHeader::Tc &tc = msg.GetTc ();
  tc.ansn = m_ansn;
...
{
  hdolsr::MessageHeader msg;
  hdolsr::MessageHeader::Mid &mid = msg.GetMid ();
...
RoutingProtocol::SendHna ()
{
  ...
  msg.SetHopCount (0);
  msg.SetMessageSequenceNumber (GetMessageSequenceNumber ());
  hdolsr::MessageHeader::Hna &hna = msg.GetHna ();

  std::vector<hdolsr::MessageHeader::Hna::Association> &associations = hna.associations;
  ...
  for (Associations::const_iterator it = localHnaAssociations.begin ();
    ...
    {
      hdolsr::MessageHeader::Hna::Association assoc = { it->networkAddr, it->netmask};
...
void
RoutingProtocol::SendHd()
{
  NS_LOG_FUNCTION(this);

  hdolsr::MessageHeader msg;

  msg.SetVTime(OLSR_HD_HOLD_TIME);
  msg.SetOriginatorAddress(m_mainAddress);
  msg.SetTimeToLive(1);
  msg.SetHopCount(0);
  msg.SetMessageSequenceNumber(GetMessageSequenceNumber());

  hdolsr::MessageHeader::HD &hd = msg.GetHd();
/*
...
*/
  ...
  //NS_LOG_DEBUG("velocity: " << myvelocity << "   x= " <<  mobility->GetVelocity().x);
  hd.SetLocation(myposition);
  hd.SetVelocity(myvelocity);
...
void
RoutingProtocol::LinkSensing (const hdolsr::MessageHeader &msg,
                              const hdolsr::MessageHeader::Hello &hello,
                              const Ipv4Address &receiverIface,
                              const Ipv4Address &senderIface)
{
  Time now = Simulator::Now ();
  ...
  for (std::vector<hdolsr::MessageHeader::Hello::LinkMessage>::const_iterator linkMessage =
...
void
RoutingProtocol::PopulateNeighborSet (const hdolsr::MessageHeader &msg,
                                      const hdolsr::MessageHeader::Hello &hello)
{
...
void
RoutingProtocol::PopulateTwoHopNeighborSet (const hdolsr::MessageHeader &msg,
                                            const hdolsr::MessageHeader::Hello &hello)
{
  Time now = Simulator::Now ();
  ...
  for (LinkSet::const_iterator link_tuple = m_state.GetLinks ().begin ();
       link_tuple != m_state.GetLinks ().end (); link_tuple++)
    {
      ...
      typedef std::vector<hdolsr::MessageHeader::Hello::LinkMessage> LinkMessageVec;
...
void
RoutingProtocol::PopulateMprSelectorSet (const hdolsr::MessageHeader &msg,
                                         const hdolsr::MessageHeader::Hello &hello)
{
  NS_LOG_FUNCTION (this);
 ...
 * 三维向量叉乘
 */
static Vector3D ThreeCross(const Vector3D &a, const Vector3D &b)
{
...
static int CalculateWillinessWithLDtime(double LD_time)
{
  double a = LD_time;
  int will = 0;
  if(a-3.0<=0)
	...
	else if(a-3.0>0 && a-10.0<=0)
	{
		will = 2;
	}	
	else if(a-10.0>0 && a-20.0<=0)
	{
		will = 3;
	}
	else if(a-20.0>0 && a-40.0<=0)
	{
		will = 4;
	}
	else if(a-40.0>0 && a-75.0<=0)
	{
		will = 5;
	}
	else if(a-75.0>0)
...
void
RoutingProtocol::PopulateNeighborWillingness (const hdolsr::MessageHeader &msg,
                                              const hdolsr::MessageHeader::HD &Hd)
{
  NeighborTuple *nb_tuple = m_state.FindNeighborTuple (msg.GetOriginatorAddress());

  //int will_change[] = {6,6,5,4,3,2,1};
  //int will_change[] = {6,6,5,4,3,2,1};
  ...
  if(nb_tuple != NULL)
  {
    ...
    double z1 = myposition.z;
    double x2 = Hd.GetLocation().x;
    double y2 = Hd.GetLocation().y;
    double z2 = Hd.GetLocation().z;
    
    double Vx1 = myvelocity.x;
    double Vy1 = myvelocity.y;
    double Vz1 = myvelocity.z;
    double Vx2 = Hd.GetVelocity().x;
    double Vy2 = Hd.GetVelocity().y;
    double Vz2 = Hd.GetVelocity().z;
    */
    //NS_LOG_UNCOND("myposition=" << myposition << " myvelocity=" << myvelocity <<
    //              " nei_position=" << Hd.GetLocation() << " nei_velocity=" << Hd.GetVelocity());
    //本节点与邻居节点的距离
    double D_12 = CalculateDistance(myposition,Hd.GetLocation());
    //NS_LOG_UNCOND("D_12=" << D_12);
    //邻居节点相对于本节点的相对速度向量
    Vector3D v_rel_21 = Hd.GetVelocity() - myvelocity;
    //NS_LOG_UNCOND("v_rel_21=" << v_rel_21);
    //相对速度的值
    double v_rel_value = v_rel_21.GetLength();
    //NS_LOG_UNCOND("v_rel_value=" << v_rel_value);
    //相对速度的单位向量，为了下面计算做准备
    Vector3D v_rel_e(v_rel_21.x/v_rel_value,v_rel_21.y/v_rel_value,v_rel_21.z/v_rel_value);
    //NS_LOG_UNCOND("v_rel_e=" << v_rel_e);
    //邻居节点相对与本节点的位置向量
    Vector3D n2_n1 = myposition - Hd.GetLocation();
    //NS_LOG_UNCOND("n2_n1=" << n2_n1);
    //本节点垂直于邻居节点移动路径的距离
    double d_2_l = ThreeCross(n2_n1,v_rel_e).GetLength();
    //NS_LOG_UNCOND("d_2_l=" << d_2_l);

    double D_12_after = CalculateDistance((myposition + myvelocity),(Hd.GetLocation() + Hd.GetVelocity()));
    double T_ld;
    if(D_12_after>D_12)
    {
      T_ld = (sqrt(MAX_RANGE*MAX_RANGE-d_2_l*d_2_l)-sqrt(D_12*D_12-d_2_l*d_2_l)) / v_rel_value;
    }
    else
    {
      T_ld = (sqrt(MAX_RANGE*MAX_RANGE-d_2_l*d_2_l)+sqrt(D_12*D_12-d_2_l*d_2_l)) / v_rel_value;
    ...
    int will = CalculateWillinessWithLDtime(T_ld);
    NS_LOG_DEBUG("populate willingness for node " << msg.GetOriginatorAddress() << " :" << T_ld << " to " << will);
    //uint8_t will = will_change[(uint8_t)(T_ld/(double)(MAX_T_LD*10))];
    //NS_LOG_UNCOND("will=" << (int)will);
    //double distance = sqrt( pow( (x1+Vx1-x2-Vx2),2 )+pow( (y1+Vy1-y2-Vy2),2 ) );
    //double d_will = sqrt( pow( (Vx1-Vx2),2 )+pow( (Vy1-Vy2),2 ) );
    //uint8_t will = will_change[(uint8_t)(d_will/WILL_TRANS_FACTOR)];
    //uint8_t will = will_change[(uint8_t)(distance/((MAX_RANGE+MAX_SPEED_DIFF)/6))];
    //NS_ASSERT(will>=OLSR_WILL_LOW && will<=OLSR_WILL_HIGH);
    //NS_LOG_DEBUG("populate willingness for node " << msg.GetOriginatorAddress() << " :" << d_will << " to " << (int)will);
    //NS_LOG_DEBUG("populate willingness for node " << msg.GetOriginatorAddress() << " :" << distance << " to " << (int)will);

    nb_tuple->willingness = will;
  }
}
...
///
void
hdolsr::mac_failed (Ptr<Packet> p)
{
  ...
  debug ("%f: Node %d MAC Layer detects a breakage on link to %d\n",
         now,
         hdolsr::node_id (ra_addr ()),
         hdolsr::node_id (ch->next_hop ()));
...
//         Simulator::Now (),
//         hdolsr::node_id(ra_addr()),
//         hdolsr::node_id(tuple->neighborMainAddr),
...
//         hdolsr::node_id(ra_addr()),
//         hdolsr::node_id(tuple->neighborMainAddr),
//         hdolsr::node_id(tuple->twoHopNeighborAddr));
...
//         Simulator::Now (),
//         hdolsr::node_id(ra_addr()),
//         hdolsr::node_id(tuple->main_addr()));
...
//         Simulator::Now (),
//         hdolsr::node_id(ra_addr()),
//         hdolsr::node_id(tuple->dest_addr()),
//         hdolsr::node_id(tuple->last_addr()),
//         tuple->seq());
...
//         hdolsr::node_id(ra_addr()),
//         hdolsr::node_id(tuple->main_addr()),
//         hdolsr::node_id(tuple->iface_addr()));
...
void
RoutingProtocol::HdTimerExpire ()
{
  SendHd ();
  m_hdTimer.Schedule (m_hdInterval);
}
